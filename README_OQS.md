...
on **Ubuntu**, run:
	...
	./Configure no-shared enable-oqs linux-x86_64 -lm
	make -j

on **macOS** (x86_64), run:

	./Configure no-shared enable-oqs darwin64-x86_64-cc
	make -j

on **macOS** (M1 / arm64), run:

	./Configure no-shared enable-oqs darwin64-arm64-cc
	make -j
...
Now we follow the standard instructions for building OpenSSL:
	...
	perl Configure VC-WIN64A enable-oqs
	nmake
